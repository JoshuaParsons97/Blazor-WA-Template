using BlazorTemplate.Server.Azure;
using BlazorTemplate.Server.MailGenerator;
...
namespace BlazorTemplate.Server.Services.AccountService
{
    public class AccountService : IAccountService
    {
        private readonly ISecurityService _securityService;
        private readonly WatemplateContext _ecommerceContext;
        private readonly IConfiguration _configuration;
        public AccountService(WatemplateContext ecommerceContext, IConfiguration configuration, ISecurityService securityService)
        {
        ...
        public async Task<ServiceResponse<bool>> ChangePassword(ChangePasswordDTO changePassword, string Email)
        {
            ...
            {
                User user = await _ecommerceContext.Users.FirstAsync(u => u.Email.ToLower() == Email.ToLower());
        ...
        public async Task<ServiceResponse<bool>> RecoverPassword(PasswordRecoveryDTO passwordRecovery)
        {
            ...
            {
                User user = await _ecommerceContext.Users.FirstAsync(u => u.Email.ToLower() == passwordRecovery.Email.ToLower());
                ...
                _ecommerceContext.PasswordRecoveries.Add(Recovery);
                await _ecommerceContext.SaveChangesAsync();
        ...
        public async Task<ServiceResponse<bool>> ResetPassword(ResetPasswordDTO resetPassword)
        {
            ...
            {
                PasswordRecovery Recovery = _ecommerceContext.PasswordRecoveries.FirstAsync(R => R.PasswordRecoveryId == RecoveryID).Result;

                if (Recovery.Expiry > DateTime.UtcNow)
                {
                    User User = await _ecommerceContext.Users.FirstAsync(U => U.UserId == Recovery.UserId);
        ...
        {
            var response = new ServiceResponse<string>();
            var user = await _ecommerceContext.Users.FirstOrDefaultAsync(user => user.Email.ToLower() == Email.ToLower());
            if (user == null)
            {
                response.Success = false;
        ...
        {
            return await _ecommerceContext.Users.AnyAsync(user => user.Email.ToLower() == email.ToLower());
        }
